...
function Crear() {
  ...
  useEffect(()=>{
    async function getData(){
      const respuesta = await Promise.all([fns.GetData(`/documentos/rutas/${empresa}`), fns.GetData(`/documentos/series/${empresa}`), fns.GetData(`facturacion/viaje/head/${empresa}`)]);
      console.log(respuesta)
  ...
  async function handleGetDoc(){
    if(serie!==''&&folio!==''){
      console.log(empresa)
      const respuesta = await fns.GetData(`facturacion/documento/${empresa}/${serie}/${folio}`)
      if(respuesta['mensaje']===undefined){
  ...
  async function handdleSubmit(e){
    ...
    if(correcto===true){
      ...
      if(docs.length<=0)alert('El viaje debe contener dcoumentos');
        ...
        const respuesta = await fns.PostData(`facturacion/viajes/crear/${empresa}`, {
          empresa:empresa,
          serie:serieViaje,
          folio:folioViaje,
